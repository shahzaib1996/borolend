...
if(isset($_GET['approve'])) {
	$query = "UPDATE apply_for_loan SET a_check='1' WHERE aid='".$aid."'";
...
if(isset($_GET['reject'])) {
	$query = "UPDATE apply_for_loan SET a_check='0' WHERE aid='".$aid."'";
	mysqli_query($con,$query);
	echo "<script>alert('Loan Request Approved')</script>";
}
...
if($type == 'bi') {
$setuser1 = $con->query("select * from borrower_individual where id='$id'");
...
$mobile = $row1->mobile;
$email = $row1->email;
$setuser2 = $con->query("select * from borrower_profile where uid='$comid'");
$row2 = $setuser2->fetch_object();
$city = $row2->pcity;
$pro = $row2->pprovince;
$setuser3 = $con->query("select * from apply_for_loan where aid='$aid'");
$row3 = $setuser3->fetch_object();
$atl = $row3->borrow_amount;
$rate = $row3->rate;
$time = $row3->time;
$a_c = $row3->a_check;
...
$setuser1 = $con->query("select * from borrower_business where id='$id'");
$row1 = $setuser1->fetch_object();
$ufn = $row1->fname;
$uln = $row1->lname;
$name = $ufn.' '.$uln;
...
<html>
  ...
  <link rel="stylesheet" href="../../bower_components/font-awesome/css/font-awesome.min.css">
	...
	<div class="col-md-12  first ">
		...
		<tr>
			...
			<tbody>
				...
				<tr style="margin-left:20px;">
					<td style="width:25%; " >Borrowerd Amount</td>
		...
		<tr>
			...
			<tbody>
				<tr style="margin-left:20px;">
					<td style="width:25%; " >Borrowed Amount</td>
					<td  > <?php echo $atl;?></td>
				</tr>
				<tr style="margin-left:20px;">
	...
	<?php
	if(true){ echo '<a href="loan_requests.php" class="btn btn-primary"><strong>Back</strong></a>';}
	else{echo '<a href="loan_requests.php" class="btn btn-primary"><strong>Back</strong></a>';}
	?>
		<?php
						if($a_c == '0') {
