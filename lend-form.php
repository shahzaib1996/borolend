...
// 	$type = $_SESSION['type'];
// 	if($type == 'bi'){
// 		$cl = $con->query("SELECT * FROM lender_individual WHERE id='$uid'");
// 		$r = $cl->fetch_object();
// 		$chk = $r->loan_check;
// 		if($chk == '0') {}
// 		else { header('location: borrower-account.php');}
...
if(isset($_POST['submit'])) {
	...
	$sql = "INSERT INTO `lend_req` (`uid`,`type`,`lend_amount`,`rate`,`time`,`payment`,`purpose`,`sdate`) VALUES ('".$auid."','".$atype."','".$atb."','".$rate."','".$time."','".$payment."','".$purpose."','".$date."')";
	if(mysqli_query($con,$sql)) {
		print "<script>alert('Your Lending Request has been submitted, you will be notified when your request has been accepted.')</script>";
		if($atype == 'li') {
			$setchk = "UPDATE lender_individual SET lend_check = '1'";
			mysqli_query($con,$setchk);
			// header('location: lender-account.php');
		}elseif($atype == 'lo') {
			$setchk = "UPDATE lender_org SET lend_check = '1'";
			mysqli_query($con,$setchk);
			// header('location: lender-account.php');
		}
	}
	else {
		print "<script>alert('Unexpected Error')</script>";
...
<html>
	<head>
	   	<title>BOROLEND | Lend Application</title>
	...
	<header class="header-4 header-2">	
		...
		<div class="container main-text">
			<div class="col-md-12">
				<h3>Lend Application</h3> <span class="location pull-right">You Are Here :  <a href="index.php">Home</a>  >  <span class="r-page">Borrower Posting Form</span></span>
			</div>
		</div>
	...
	<div class="al-r">
		...
		<img src="images/pencil-3.png" class="p2" alt="" />
			<div class="col-md-6 login">
				...
				<input type="text" class="form-control" placeholder="Enter Amount to Lend..." name="atb" required>
